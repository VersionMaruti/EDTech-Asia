...
import studyNotionLogo from '../../assets/Logo/edtech-asia-logo-vector.png'
...
const Navbar = () => {
    ...
    return (
        <nav className={`z-[10] bg-white flex h-24 w-full items-center justify-center border-b-[1px] border-b-richblack-700 text-black translate-y-0 transition-all `}>
                                    ...
                                    link.title === "Catalog" ? (
                                        <div
                                            className={`group relative flex cursor-pointer items-center gap-1 ${matchRoute("/catalog/:catalogName")
                                                ? " bg-white text-blue-800 rounded-xl p-1 px-3"
                                                : "text-blue-800 rounded-xl p-1 px-3"
                                                }`}
                                        >
                                            <p className='bg-white text-blue-800'>{link.title}</p>
                                            <MdKeyboardArrowDown />
                                            {/* drop down menu */}
                                        ...
                                        <Link to={link?.path}>
                                            <p className={`${matchRoute(link?.path) ? "bg-white text-[#0022ff]" : "text-black"} rounded-xl p-1 px-3 `}>
                                                {link.title}
                                            </p>
                                        </Link>)
                                }
                    ...
                    }
                    {
                        token === null && (
                            <Link to="/login">
                                ...
                                <button className={` px-[12px] py-[8px] text-black rounded-md h-14 w-36
                                 ${matchRoute('/login') ? 'border-[2.5px] border-[#1600ff]' : ' bg-white'} `}
                                >
                                    Log in
                                </button>
                            </Link>
                    ...
                    }
                    {
                        token === null && (
                            <Link to="/signup">
                                ...
                                <button className={` px-[12px] py-[8px] text-white rounded-md h-14 w-36 bg-[#2847b0]
